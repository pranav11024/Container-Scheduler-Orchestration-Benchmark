...
func NewNode(name string, cpu, memory, network, io float64) *Node {
	...
	if err != nil {
		panic(fmt.Sprintf("failed to create Docker manager for node %s: %v", name, err))
	}
...
func (n *Node) AddContainer(c *container.Container) bool {
	...
	containerID, err := n.dockerManager.RunContainer(c)
	if err != nil {
		return false
	}
	
	c.SetDockerID(containerID)
...
func (n *Node) RemoveContainer(containerID string) bool {
	for i, c := range n.containers {
		if c.ID() == containerID {
			// Stop and remove the actual Docker container
			if err := n.dockerManager.StopContainer(c.DockerID()); err != nil {
				return false
			}
