...
def create_plots(all_dfs, all_metrics, results_dir):
    ...
    for name, df in all_dfs.items():
        ...
        df_sorted['bucket'] = pd.qcut(df_sorted.index, 20, labels=False)
        util_over_time = df_sorted.groupby('bucket')['ResourceUtilization'].mean()
        plt.plot(util_over_time.index, util_over_time.values, 'o-', label=name)
